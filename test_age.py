...
def test(test_dir, try_dir, save_dir, epoch_dir, args):
    ...
    # -- model
    num_classes_age = json_data['num_classes_age']
    model_name = json_data['model_age']
    model_module = getattr(import_module('model'), model_name)
    model = model_module(
        num_classes=num_classes_age,
        test=True
    ).to(device)
    ...
    for idx, test_batch in enumerate(tqdm(test_loader)):
        ...
        for pred, file_path in zip(preds, file_paths):
            ...
            eval_df.loc[person_idx[0], 'age'] = pred
...
if __name__ == '__main__' :
    ...
    try_dir = os.path.join(try_dir,'age')
