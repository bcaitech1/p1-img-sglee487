...
def test(test_dir, try_dir, save_dir, epoch_dir, args):
    ...
    # -- model
    num_classes_gender = json_data['num_classes_gender']
    model_name = json_data['model_gender']
    model_module = getattr(import_module('model'), model_name)
    model = model_module(
        num_classes=num_classes_gender,
        test=True
    ).to(device)
    ...
    for idx, test_batch in enumerate(tqdm(test_loader)):
        ...
        for pred, file_path in zip(preds, file_paths):
            pred = int(pred.cpu().clone())
            file_name = file_path.split('/')[-1]
            person_idx = eval_df.loc[eval_df['ImageID'] == file_name].index
            eval_df.loc[person_idx[0], 'gender'] = pred

    save_csv(eval_df,save_dir,epoch_dir.split('/')[-1].split('.pth')[0])
...
if __name__ == '__main__' :
    ...
    try_dir = os.path.join(try_dir,'gender')

    epoch_dir = find_dir_epoch(epoch, try_dir)
    save_dir = try_dir

    test(data_dir, try_dir, save_dir, epoch_dir, args)