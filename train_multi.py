...
def train(data_dir, model_dir, args):
    ...
    # -- model
    models = []
    model_module_mask = getattr(import_module("model"), args.model_mask)  # default: BaseModel
    model_mask = model_module_mask(
        num_classes=3,
    ...
    if args.optimizer == "AdamP":
        optimizer_mask = AdamP(filter(lambda p: p.requires_grad, model_mask.parameters()),
                          lr=args.lr,
                          weight_decay=5e-4)
    else:
    ...
    criterion_gender = create_criterion(args.criterion_gender)  # default: cross_entropy
    if args.optimizer == "AdamP":
    ...
    # -- logging
    logger_mask = SummaryWriter(log_dir=os.path.join(save_dir,'mask'))
    ...
    for epoch in range(args.epochs):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            preds_age = torch.tensor(outs_age,dtype=np.int32)
            loss_mask = criterion_mask(outs_mask, labels_mask)
            ...
            if (idx + 1) % args.log_interval == 0:
                ...
                print(
                    ...
                    f"training loss {train_loss_mask:4.4} || training accuracy {train_acc_mask:4.2%} || lr {current_lr_mask}"
                )
                logger_mask.add_scalar("Train/loss", train_loss_mask, epoch * len(train_loader) + idx)
                logger_mask.add_scalar("Train/accuracy", train_acc_mask, epoch * len(train_loader) + idx)
                print(
                    f"Epoch[{epoch}/{args.epochs}]({idx + 1}/{len(train_loader)}) || "
                    f"training loss {train_loss_gender:4.4} || training accuracy {train_acc_gender:4.2%} || lr {current_lr_mask}"
                )
                ...
                print(
                    f"Epoch[{epoch}/{args.epochs}]({idx + 1}/{len(train_loader)}) || "
                    f"training loss {train_loss_age:4.4} || training accuracy {train_acc_age:4.2%} || lr {current_lr_mask}"
                )
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                loss_item_mask = criterion_mask(outs_mask, labels_mask).item()
                acc_item_mask = (labels_mask == preds_mask).sum().item()
                val_loss_items_mask.append(loss_item_mask)
                val_acc_items_mask.append(acc_item_mask)
                ...
                if figure is None:
                    ...
                    figure = grid_image(inputs_np, (labels_mask, labels_gender, labels_age), (preds_mask, preds_gender, preds_age), args.dataset != "MaskSplitByProfileDataset")
                    plt.show()

            val_loss_mask = np.sum(val_loss_items_mask) / len(val_loader)
            val_acc_mask = np.sum(val_acc_items_mask) / len(val_set)
            if val_loss_mask < best_val_loss_mask or val_acc_mask > best_val_acc_mask:
                save_model(model_mask,epoch,val_loss_mask,val_acc_mask,os.path.join(save_dir,"mask"),args.model_mask)
                if val_loss_mask < best_val_loss_mask and val_acc_mask > best_val_acc_mask:
                    print(f"New best model_mask for val acc and val loss : {val_acc_mask:4.2%} {val_loss_mask:4.2}! saving the best model_mask..")
                    best_val_loss_mask = val_loss_mask
                    best_val_acc_mask = val_acc_mask
                elif val_loss_mask < best_val_loss_mask:
                    print(f"New best model_mask for val loss : {val_loss_mask:4.2}! saving the best model_mask..")
                    save_model(model_mask, epoch, val_loss_mask, val_acc_mask, save_dir, args.model_mask)
                    best_val_loss_mask = val_loss_mask
                elif val_acc_mask > best_val_acc_mask:
                    print(f"New best model_mask for val accuracy : {val_acc_mask:4.2%}! saving the best model_mask..")
                    save_model(model_mask,epoch,val_loss_mask,val_acc_mask,save_dir,args.model_mask)
                    best_val_acc_mask = val_acc_mask
            ...
            if val_loss_gender < best_val_loss_gender or val_acc_gender > best_val_acc_gender:
                save_model(model_gender, epoch, val_loss_gender, val_acc_gender, os.path.join(save_dir, "mask"),
                           args.model_gender)
                if val_loss_gender < best_val_loss_gender and val_acc_gender > best_val_acc_gender:
                    ...
                    save_model(model_gender, epoch, val_loss_gender, val_acc_gender, save_dir, args.model_gender)
                    best_val_loss_gender = val_loss_gender
            ...
            if val_loss_age < best_val_loss_age or val_acc_age > best_val_acc_age:
                save_model(model_age, epoch, val_loss_age, val_acc_age, os.path.join(save_dir, "mask"),
                           args.model_age)
                if val_loss_age < best_val_loss_age and val_acc_age > best_val_acc_age:
                    ...
                    save_model(model_age, epoch, val_loss_age, val_acc_age, save_dir, args.model_age)
                    best_val_loss_age = val_loss_age
            ...
            print(
                f"[Val] acc: {val_acc_mask:4.2%}, loss: {val_loss_mask:4.2} || "
                f"best acc: {best_val_acc_mask:4.2%}, best loss: {best_val_loss_mask:4.2}"
            )
            logger_mask.add_scalar("Val/loss", val_loss_mask, epoch)
            logger_mask.add_scalar("Val/accuracy", val_acc_mask, epoch)
            logger_mask.add_figure("results", figure, epoch)
            print(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_mask', type=str, default='BaseModel',
                        help='mask model type (default: BaseModel) (BaseModel, Vgg19BasedModel, EfficientNet_b4)')
    parser.add_argument('--criterion_mask', type=str, default='cross_entropy', help='mask loss function for training (default cross_entropy) (cross_entropy, focal, label_smoothing, f1)')
    ...
    parser.add_argument('--criterion_gender', type=str, default='cross_entropy', help='gender loss function for training (default cross_entropy) (cross_entropy, focal, label_smoothing, f1)')
    ...
    parser.add_argument('--criterion_age', type=str, default='mse', help='age loss function for training (default mse) (cross_entropy, focal, label_smoothing, f1)')
